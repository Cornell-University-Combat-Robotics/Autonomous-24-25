...
class Ram():
    ...
    def ram_ram(self, bots = {'huey': {'bbox': list, 'center': list, 'orientation': float}, 'enemy': {'bbox': list, 'center': list}}):
        ...
        # print("speed: ", speed)
        # print("turn:", turn)
        return self.huey_move(speed, turn), self.enemy_future_positions

# Predicts future position of enemy bot(s) while considering acceleration
    def get_future_pos(self, enemy_previous_positions, enemy_position):
        '''
        1. Take prev1, prev2
        2. Find v0 from prev1, prev2
        3. Find vf from prev1, curr
        4. Find a from  v0,  vf (a = (vf - v0) / t)
        5. Predict future position by adding displacement (1/2)at^2+v0t
        '''
