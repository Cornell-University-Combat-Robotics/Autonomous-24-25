...
class ConvNeuralNet(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        housebot_mask = labels['labels'] == 0  # Mask for housebot
        robot_mask = labels['labels'] == 1     # Mask for robots
        ...
        if labels is not None:
            # Housebot losses
            housebot_boxes = labels['boxes'][housebot_mask]
            housebot_scores = class_scores[housebot_mask]
            housebot_preds = bounding_boxes[housebot_mask]
            
            housebot_class_loss = (
                F.cross_entropy(housebot_scores, labels['labels'][housebot_mask])
                if housebot_scores.numel() > 0 else torch.tensor(0.0, device=x.device)
            )
            housebot_box_loss = (
                F.smooth_l1_loss(housebot_preds, housebot_boxes, reduction='mean')
                if housebot_preds.numel() > 0 else torch.tensor(0.0, device=x.device)
            )

            robot_boxes = labels['boxes'][robot_mask]
            robot_scores = class_scores[robot_mask]
            robot_preds = bounding_boxes[robot_mask]

            robot_class_loss = (
                F.cross_entropy(robot_scores, labels['labels'][robot_mask])
                if robot_scores.numel() > 0 else torch.tensor(0.0, device=x.device)
            )
            robot_box_loss = (
                F.smooth_l1_loss(robot_preds, robot_boxes, reduction='mean')
                if robot_preds.numel() > 0 else torch.tensor(0.0, device=x.device)
            ) 
                    
            housebot_box_loss /= 600  # Scale housebot regression loss
            robot_box_loss /= 600     # Scale robot regression loss

            # Apply custom weights
            housebot_loss_weight = 0.5  # Weight housebot loss less
            ...
            return {
                ...
                "loss_box_reg_housebot": housebot_box_loss,
                "loss_classifier_robot": robot_class_loss,
                "loss_box_reg_robot": robot_box_loss,
