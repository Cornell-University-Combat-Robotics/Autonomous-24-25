...
def train(model, num_epochs=10, learning_rate=0.0001):
    ...
    training_loader = DataLoader(train_dataset, batch_size = 8, shuffle=True, collate_fn=collate_fn)
    validation_loader = DataLoader(val_dataset, batch_size = 8, shuffle=False, collate_fn=collate_fn)
    ...
    def loader_loss(images, labels):
        ...
        if isinstance(outputs, dict):
            housebot_class_loss = outputs['loss_classifier_housebot'].mean()
            housebot_box_loss = outputs['loss_box_reg_housebot'].mean()
            robot_class_loss = outputs['loss_classifier_robot'].mean()
            robot_box_loss = outputs['loss_box_reg_robot'].mean()
