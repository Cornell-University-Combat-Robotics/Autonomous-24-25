    ...
    def detect_our_robot_main(self, bot_images: list[np.ndarray]):
        ...
        try:
            ...
            if bot_images and all(img is not None for img in bot_images):
                ...
                our_bot_down = self.find_our_bot(bot_images, self.selected_colors[3])
                if our_bot_down is None:
                    our_bot = self.find_our_bot(bot_images, self.selected_colors[0])
                else:
                    our_bot = our_bot_down
                    self.IS_FLIPPED = -1
                end_time = time.time()
                print(
